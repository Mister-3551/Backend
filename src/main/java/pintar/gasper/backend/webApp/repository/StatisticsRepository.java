...
public interface StatisticsRepository extends JpaRepository<PlayerStatistics, String> {
    ...
    @Query(value = "SELECT u.username, u.rank, u.current_xp, u.next_level_xp, s.kills, s.deaths, s.wins, s.losses, s.highest_streak, s.most_kills, CASE WHEN s.deaths = 0 THEN s.kills ELSE ROUND(s.kills / s.deaths, 1) END as ratio " +
            "FROM statistics s " +
    ...
    @Query(value = "SELECT u.id, u.username, u.rank, s.kills " +
            "FROM users u " +
